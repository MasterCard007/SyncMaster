...
def compare_and_sync(source, target, executor):
    ...
    for future in tqdm(concurrent.futures.as_completed(future_to_file), total=len(source_files), desc="Comparing files"):
...
def sync_folders(source, target):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
        ...
        if proceed == 'y':
            with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as copy_executor:
                future_to_file = {}
                for file in to_transfer:
                    ...
                    future_to_file[copy_executor.submit(copy_file, file, target_path)] = file

                for future in tqdm(concurrent.futures.as_completed(future_to_file), total=len(future_to_file), desc="Copying files"):
                    try:
                        future.result()
                    except Exception as e:
